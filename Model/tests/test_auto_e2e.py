...
class TestFeatureFusionComponent:
    def test_output_shape(self, device):
        ...
        # Simulate SwinV2 multi-scale outputs for B*V=16 (B=2, V=8)
        features = [
            torch.randn(16, 56, 56, 96, device=device),   # stage 0
            torch.randn(16, 28, 28, 192, device=device),  # stage 1
            torch.randn(16, 14, 14, 384, device=device),  # stage 2
            torch.randn(16, 7, 7, 768, device=device),    # stage 3
        ]
...
class TestCrossAttentionFusion:
    def test_output_shape(self, device):
        ...
        x = torch.randn(16, 1440, 7, 7, device=device)  # B*V=16
