...
e2e_test: deps ## real-data training tests (auto-downloads nvidia if HF_TOKEN is set)
...
.PHONY: setup setup-map setup-local deps deps-map lint typecheck test test-local test-local-all test-local-map test-local-integration ci benchmark e2e_test help
